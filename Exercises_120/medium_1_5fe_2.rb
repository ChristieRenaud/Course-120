...
class Minilang
  ...
  def eval(sides)
    format(@commands, sides).split.each do |command|
      if command =~ /\A[-+]?\d+\z/
...
sides_1 = {length: 10, width: 13}
sides_2 = {length: 12, width: 88}
sides_3 = {length: 1, width: 3}
minilang.eval(sides_1)
minilang.eval(sides_2)
minilang.eval(sides_3)
